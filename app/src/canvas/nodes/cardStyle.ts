...
export const CARD_BASE =
  'owf-card relative overflow-visible rounded-2xl border font-sans ' +
...
export function cardClass(selected: boolean): string {
  return selected ? `${CARD_BASE} owf-card-selected` : CARD_BASE;
}
...
export function cardWrapperStyle({
  ...
  (style as Record<string, string | number | undefined>)['--owf-node-accent'] =
    accent;
