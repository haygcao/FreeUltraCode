# Claude Code में Dynamic Workflows आ गए हैं। बाकी मॉडलों का क्या? FreeUltraCode एक ओपन-सोर्स विकल्प है
...
इसी सोच के आधार पर मैंने FreeUltraCode बनाया, या अधिक सही कहें तो AI ने बनाया। यह Claude Code जैसे workflows को visual canvas में बदलता है और कोशिश करता है कि वही flow Claude Code, Codex, Gemini और दूसरे local या cloud runtimes पर भी चल सके।

इस बार मैं अमूर्त अवधारणाओं की बात नहीं करूंगा। सीधे screenshots के हिसाब से चलूंगा। उदाहरण भी ठोस है: FreeUltraCode को कई interface styles का समर्थन देना, Pencil को default बनाना, और Settings / Appearance में style बदलने देना।

विकास के दौरान मैंने अधिक से अधिक काम FreeUltraCode के अंदर ही करने की कोशिश की, ताकि यह खुद को bootstrap कर सके।
...
FreeUltraCode के main interface में बीच में workflows blueprint है, दाईं ओर node properties हैं, और नीचे AI input और output है।
...
```text
मैं चाहता हूं कि FreeUltraCode कई interface styles support करे,
default में Pencil design इस्तेमाल करे,
और Settings / Appearance में इन्हें switch किया जा सके।
```

लिखने के बाद Ctrl+Enter दबा सकते हैं, या नीचे दाईं ओर send button दबा सकते हैं। FreeUltraCode इस natural language को editable workflows blueprint में बदल देता है।
...
Request भेजने के बाद FreeUltraCode पहले current step को एक complete workflow में reorganize करता है।
...
यह भी बताता है कि FreeUltraCode सिर्फ boxes draw नहीं कर रहा। Canvas के पीछे unified workflows structure है, इसलिए आगे अलग-अलग runtimes जोड़े जा सकते हैं।
...
FreeUltraCode development complete होने के बाद program restart करें, और Settings / Appearance में अलग-अलग appearance styles switch करें।
...
FreeUltraCode की सबसे बड़ी value prompt के ऊपर UI लगाने में नहीं है।
...
workflows का पूरा concept अभी काफी शुरुआती है, और FreeUltraCode भी अभी शुरू ही हुआ है। Runtime adapters, node capabilities और script ecosystem बदलते रहेंगे।
...
```bash
fuc gen "एक code review process बनाओ" -o review.js   # एक वाक्य से workflow script generate करें
fuc gen review.js "इसमें एक security review node और जोड़ो"  # एक वाक्य से मौजूदा script modify करें
fuc run review.js                          # script को run करें
```

बस यही तीन उपयोग हैं (असल में commands दो ही हैं — `gen` और `run`)।

**`fuc gen`** natural language से workflow generate या modify करता है। यह interface के नीचे वाले AI input box जैसी ही capability है: आप requirement बताते हैं, यह script generate करता है; आप किसी मौजूदा script की तरफ इशारा करके बताते हैं कि क्या बदलना है, और यह बदल देता है।

यहां एक अहम बात है: **यह zero-config है, आपको कोई API Key नहीं भरना पड़ता**। क्योंकि यह आपके local machine पर पहले से login किए हुए `claude` CLI को ही reuse करता है (और runtime में भी यही path उपयोग होता है), तो जब तक आपके पास claude installed है और आप login हैं, `fuc gen` सीधे काम करता है। अगर installed नहीं है तो यह आपको पहले `claude login` करने की याद दिलाता है।

**`fuc run`** script को run करता है, node-by-node execute करता है, और terminal में real-time progress print करता है:
